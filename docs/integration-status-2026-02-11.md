...
### Валидация билета (QR)
- Страница проверки: `/ticket/validate`.
- QR в PDF ведет на ссылку валидации с параметрами билета.
- Доступ к данным билета и погашению только для авторизованного админа.
- При погашении в `orders` записываются:
  - `ticket_redeemed_at`,
  - `ticket_redeemed_by`.
- Для неадминского доступа предусмотрен редирект на страницу спектакля.
...
- Для быстрой синхронизации дефолтных YAML в БД используется:
  - `npm run schedule:sync:yaml`
  - импортирует только `zlata` и `marita`, очищает `demo`.
...
- `docs/sql/2026-02-11-add-ticket-redemption.sql`
...
4. Пробная покупка:
   ...
   - PDF вложен.
5. Проверка QR:
   - открыть QR из билета,
   - авторизоваться админом,
   - погасить билет,
   - убедиться, что повторно билет отмечен как уже погашенный.
