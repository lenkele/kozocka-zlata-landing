...
// Имена файлов оставляем точно такими же, как на диске: одна .jpg, остальные .JPG.
...
const CAROUSEL_PHOTOS: CarouselPhoto[] = PHOTO_FILES.map((file, index) => ({
  src: `${PHOTOS_BASE}/${file}`,
  alt: `Кадр спектакля «Гефилте Лид» ${index + 1}`,
}));

const GALLERY_PHOTOS: GalleryPhoto[] = PHOTO_FILES.map((file, index) => ({
  src: `${PHOTOS_BASE}/${file}`,
  alt: `Кадр спектакля «Гефилте Лид» ${index + 1}`,
}));
